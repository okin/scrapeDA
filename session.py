# coding: utf-8
import dataset,urllib2,sqlalchemy
from bs4 import BeautifulSoup
import datetime,dateutil.parser,code
...
starturl=base_url+"recherche.php?suchbegriffe=&select_gremium=&datum_von="+scrape_from+"&datum_bis="+scrape_to+"&startsuche=Suche+starten"
...
def getSIDsOfMeetings():
 ...
 # prevent infinite loop
   notempty=0
   table = BeautifulSoup(getURL(starturl+"&entry="+str(entry-1))).find('table',{ "width" : "100%" }) 
...
def getSession(sid):
 ...
 # TITEL 
 session['title'] = soup.find('b', { 'class' : 'Suchueberschrift'}).get_text().encode("utf-8").decode("iso-8859-1")
 #METADATEN
 ...
 for top in tops:
  ...
  if '[Vorlage: ' in top[4]:
   if '[Vorlage: SV-' in top[4]:
    ...
    vorlnr= top[4][len("Vorlage: SV-")+6:len("Vorlage: SV-")+10]
   else:
